...
def save_model(model, optimizer, checkpoints_dir, suff=''):
    if torch.cuda.device_count() > 1:
        torch.save(model.module.state_dict(), os.path.join(checkpoints_dir, 'model' + suff + '.ckpt'))
    else:
        torch.save(model.state_dict(), os.path.join(checkpoints_dir, 'model' + suff + '.ckpt'))
    torch.save(optimizer.state_dict(), os.path.join(checkpoints_dir, 'optim' + suff + '.ckpt'))
...
def main(args):
    ...
    if params_cnn is not None and args.finetune_after == 0:
        optimizer = torch.optim.Adam([{'params': params}, {'params': params_cnn, 'lr': args.learning_rate * args.scale_learning_rate_cnn}],
    ...
    for epoch in range(start, args.num_epochs):
        ...
        if args.decay_lr:
            ...
            print(f'Epoch {epoch}. lr: {new_lr:.5f}')
            set_lr(optimizer, decay_factor)

        # Fine-tune CNN if needed
        ...
        for split in ['train', 'val']:
            ...
            total_loss_dict = {'recipe_loss': [], 'ingr_loss': [], 'eos_loss': [], 'loss': [], 'iou': [], 'perplexity': [], 'iou_sample': [], 'f1': [], 'card_penalty': []}
                ...
                if args.log_step != -1 and i % args.log_step == 0:
                    ...
                    for k in total_loss_dict.keys():
                        if len(total_loss_dict[k]) == 0:
                            continue
                        this_one = f"{k}: {np.mean(total_loss_dict[k][-args.log_step:]):.4f}"
                        lossesstr += this_one + ', '

                    strtoprint = f'Split: {split}, Epoch [{epoch}/{args.num_epochs}], Step [{i}/{total_step}], Losses: {lossesstr} Time: {elapsed_time:.4f}'
                    print(strtoprint)
            ...
            if split == 'val' and not args.recipe_only:
                ...
                compute_metrics(ret_metrics, error_types, ['accuracy', 'f1', 'jaccard', 'f1_ingredients', 'dice'], eps=1e-10, weights=None)
                total_loss_dict['f1'] = ret_metrics['f1']

            if args.tensorboard:
                logger.scalar_summary(mode=split, epoch=epoch,
                                      **{k: np.mean(v) for k, v in total_loss_dict.items() if v})

        # Save the model's best checkpoint if performance improved
